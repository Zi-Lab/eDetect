...
img_draw_object      = imread('draw_object.tif');
img_draw_object      = double(img_draw_object(:,:,1:3)) / 255;
img_create_objects   = imread('create_objects.tif');
img_create_objects   = double(img_create_objects(:,:,1:3)) / 255;
img_remove_objects   = imread('remove_objects.tif');
img_remove_objects   = double(img_remove_objects(:,:,1:3)) / 255;
img_draw_division    = imread('draw_division.tif');
img_draw_division    = double(img_draw_division(:,:,1:3)) / 255;
img_divide_objects   = imread('divide_object.tif');
img_divide_objects   = double(img_divide_objects(:,:,1:3)) / 255;
...
%%
img_segmentation = imread('cellsegmentation.tif');
img_segmentation = double(img_segmentation(:,:,1:3)) / 255;
img_feature      = imread('featureextraction.tif');
img_feature      = double(img_feature(:,:,1:3)) / 255;
img_tracking     = imread('celltracking.tif');
img_tracking     = double(img_tracking(:,:,1:3)) / 255;
img_lineage      = imread('celllineagereconstruction.tif');
img_lineage      = double(img_lineage(:,:,1:3)) / 255;
...
%%
hMain.toggletool_overlay         = uitoggletool(hMain.toolbar_manual_correction,'CData',img_change_overlay   , 'TooltipString','Change overlay'      , 'Separator' , 'on'  , 'Enable', 'off' ,'ClickedCallback',@CallbackToggleOverlay);
...
hMain.pushtool_draw_object       = uipushtool(  hMain.toolbar_manual_correction,'CData',img_draw_object      , 'TooltipString','Draw an object'      , 'Separator' , 'off' , 'Enable', 'off' ,'ClickedCallback',@DrawObject);
hMain.pushtool_create_objects    = uipushtool(  hMain.toolbar_manual_correction,'CData',img_create_objects   , 'TooltipString','Create objects'      , 'Separator' , 'off' , 'Enable', 'off' ,'ClickedCallback',@EditObjects);
hMain.pushtool_remove_objects    = uipushtool(  hMain.toolbar_manual_correction,'CData',img_remove_objects   , 'TooltipString','Remove objects'      , 'Separator' , 'off' , 'Enable', 'off' ,'ClickedCallback',@EditObjects);
hMain.pushtool_draw_division     = uipushtool(  hMain.toolbar_manual_correction,'CData',img_draw_division    , 'TooltipString','Draw division'       , 'Separator' , 'off' , 'Enable', 'off' ,'ClickedCallback',@DrawDivision);
hMain.pushtool_divide_objects    = uipushtool(  hMain.toolbar_manual_correction,'CData',img_divide_objects   , 'TooltipString','Divide objects'      , 'Separator' , 'off' , 'Enable', 'off' ,'ClickedCallback',@EditObjects);
...
%%
hMain.pushtool_cellsegmentation   = uipushtool(hMain.toolbar_pipeline,'CData',img_segmentation , 'TooltipString','Cell segmentation'             , 'Separator' , 'off' , 'Enable', 'off'   ,'ClickedCallback',@CellSegmentation);
hMain.pushtool_featureextraction  = uipushtool(hMain.toolbar_pipeline,'CData',img_feature      , 'TooltipString','Feature extraction'            , 'Separator' , 'off' , 'Enable', 'off'   ,'ClickedCallback',@FeatureExtraction);
hMain.pushtool_celltracking       = uipushtool(hMain.toolbar_pipeline,'CData',img_tracking     , 'TooltipString','Cell tracking'                 , 'Separator' , 'off' , 'Enable', 'off'   ,'ClickedCallback',@CellTracking);
hMain.pushtool_celllineage        = uipushtool(hMain.toolbar_pipeline,'CData',img_lineage      , 'TooltipString','Cell lineage reconstruction'   , 'Separator' , 'off' , 'Enable', 'off'   ,'ClickedCallback',@CellLineageReconstruction);
...
%%
set(hMain.fig,'WindowKeyPressFcn',@keyPressCallback);
param.hMain = hMain;
